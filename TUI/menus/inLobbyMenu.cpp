...
std::string gameDescription(const GameMode& gameMode) {
...
void inLobbyMenu() {
    ...
    auto LeaveButton = Button("Leave Lobby", [&screen] {
    ...
    // Demander à Roberto pour savoir commment ça marche | Commentaires + liaison au serveur à faire
    auto GameModeBox = Renderer(leftMenuButtonsContainer, [&]{
        return vbox({
            text("Game Modes") | color(Color::Green1),
            hbox({
                ClassicButton->Render() | color(Color::Green1),
                DuelButton->Render()    | color(Color::Green1),
                RoyaleButton->Render()  | color(Color::Green1)
            }),
            text(gameDescription(gameMode)) | color(Color::Green1)
        }) | border;
    });
    ...
    auto leftMenuRenderer = Renderer(leftMenuButtonsContainer, [&]{
        return window(text("Lobby : " + data.lobbyName) | color(Color::Green1),
            vbox({
                GameModeBox->Render()   | color(Color::Green1),
                PlayersBox->Render()    | color(Color::Green1),
                SpectatorsBox->Render() | color(Color::Green1),
                hbox({
                    JoinSpectatorButton->Render() | color(Color::Green1),
                    JoinPlayerButton->Render()    | color(Color::Green1)
                }),
                hbox({
                    ReadyButton->Render() | color(Color::Green1),
    ...
    auto messagesRenderer = Renderer([&]() -> Element {
        ...
        return vbox({
            window(text("Messages with " + selectedFriend) | color(Color::Green1),
                   vbox(std::move(lines)) | color(Color::Green1)) | color(Color::Green1),
            separator(),
            hbox({ messageInput->Render() | size(WIDTH, EQUAL, 30) | color(Color::Green1) , sendButton->Render() | color(Color::Green1) })
        }) | color(Color::Green1) ;
    });
