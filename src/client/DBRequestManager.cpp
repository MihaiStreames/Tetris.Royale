...
std::string DBRequestManager::toJSON(const boost::property_tree::ptree &pt) {
    std::ostringstream oss;
    write_json(oss, pt, false);
    return oss.str();
}

DBResponse DBRequestManager::sendRequest(
    http::verb method,
...
DBResponse DBRequestManager::getPlayer(const std::string &accountID) {
    return sendRequest(http::verb::get, "/get_player?accountID=" + accountID, "");
}

DBResponse DBRequestManager::getLeaderboard(const int limit) {
    return sendRequest(http::verb::get, "/get_leaderboard?limit=" + std::to_string(limit), "");
}

DBResponse DBRequestManager::getMessages(
    const std::string &accountID,
    const std::string &otherAccountID
) {
    return sendRequest(http::verb::get, "/get_messages?accountID=" + accountID + "&otherAccountID=" + otherAccountID,
                       "");
}

DBResponse DBRequestManager::registerPlayer(
...
DBResponse DBRequestManager::loginPlayer(
    const std::string &userName,
    const std::string &password
) {
    boost::property_tree::ptree pt;
...
DBResponse DBRequestManager::updatePlayer(
    const std::string &accountID,
...
DBResponse DBRequestManager::postPlayerScore(
    const std::string &accountID,
    const int score
) {
    boost::property_tree::ptree pt;
...
DBResponse DBRequestManager::sendFriendRequest(
...
DBResponse DBRequestManager::acceptFriendRequest(
    const std::string &receiverID,
    const std::string &senderID
) {
    boost::property_tree::ptree pt;
...
DBResponse DBRequestManager::declineFriendRequest(
    const std::string &receiverID,
    const std::string &senderID
) {
    boost::property_tree::ptree pt;
...
DBResponse DBRequestManager::removeFriend(
    const std::string &accountID,
...
DBResponse DBRequestManager::postMessage(
    const std::string &senderID,
    const std::string &receiverID,
...
DBResponse DBRequestManager::deleteMessage(const std::string &messageID) {
    boost::property_tree::ptree pt;
