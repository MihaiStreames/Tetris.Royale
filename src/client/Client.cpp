...
main()
{
    ...
    while (running)
    {
        ...
        case 12:
            std::cout << "Enter game mode (0 for CLASSIC, 1 for ROYALE, 2 "
                         "for DUEL): ";
            std::cin >> choice;
            ...
            std::cin >> isPublic;
            clientSession.createAndJoinLobby(static_cast<GameMode>(choice),
                                             maxPlayers, isPublic);
            break;
        ...
        case 18:
        {
            ...
            std::map<int, Action> actionMap = {
                {0, Action::MoveLeft},         {1, Action::MoveRight},
                {2, Action::MoveDown},         {3, Action::RotateLeft},
                {4, Action::RotateRight},      {5, Action::UseBag},
                {6, Action::UseBonus},         {7, Action::UseMalus},
                {8, Action::InstantFall},      {9, Action::SeePreviousOpponent},
                {10, Action::SeeNextOpponent},
            };

            std::cout << "Enter action (0 for MoveLeft, 1 for MoveRight, 2 "
                         "for MoveDown, 3 for "
                         "RotateLeft, 4 for RotateRight, 5 for UseBag, 6 "
                         "for UseBonus, 7 for "
                         "UseMalus, 8 for InstantFall), 9 for "
                         "SeePreviousOpponent, 10 for "
                         "SeeNextOpponent: ";
            std::cin >> choice;
        ...
        case 19:
        {
            ...
            // print the board
            for (size_t i = 0; i < state.playerGrid.size(); i++)
            {
                std::cout << "|";
                for (size_t j = 0; j < state.playerGrid[i].size(); j++)
                {
                    std::cout << (state.playerGrid[i][j] ? "█" : ".");
                }
                std::cout << "|" << "             " << "|";
                for (size_t j = 0; j < state.targetGrid[i].size(); j++)
                {
