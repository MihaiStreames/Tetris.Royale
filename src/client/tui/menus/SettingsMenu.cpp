...
settingsMenu()
{
    ...
    auto component = Container::Vertical({back_button});

    // We'll generate a list from data.keyBindings
    std::vector<std::string> orderedKeys = {
        "Left", "Right", "Rotate", "Drop", "Switch Player", "Power-up"};
    ...
    for (auto& move : orderedKeys)
    {
        commandList.push_back(
            border(hbox({text(move) | color(Color::Green1), separator(),
                         text("(" + data.keyBindings[move] + ")") |
                             color(Color::Green1)})));
    }

    auto buttonBox =
        Renderer(component, [&] { return hbox(back_button->Render()); });
    ...
    auto halfbottomBox = Renderer(
        ...
        [&]
        {
            return hbox(
                {filler(),
                 vbox({filler(), hbox({filler(), buttonBox->Render()})})});
        });
