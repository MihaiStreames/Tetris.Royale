...
preLobbyMenu()
{
        ...
        [&]
        {
            ...
            for (auto& lobby : data.lobbies)
            {
                lines.push_back(hbox(
                    {text(lobby.lobbyID + " - Mode: " + lobby.gameMode +
                          " - Players: " + std::to_string(lobby.nbPlayers) +
                          " - Spectators: " +
                          std::to_string(lobby.nbSpectators) +
                          " - Status: " + lobby.status) |
                     color(Color::Green1) | border}));
            }

            return window(text("Public Lobbies"), vbox(std::move(lines))) |
                   color(Color::Green1);
        });

    auto leftMenuButtonsContainer =
        Container::Vertical({SoloButton, CreateButton, LeaveButton});

    auto leftMenuRenderer = Renderer(
        leftMenuButtonsContainer,
        [&]
        {
            return vbox({SoloButton->Render() | color(Color::Green1),
    ...
    auto messagesRenderer = Renderer(
        [&]() -> Element
        {
            ...
            {
                return window(text("Messages"), text("No friends available.") |
                                                    color(Color::Red));
            }
