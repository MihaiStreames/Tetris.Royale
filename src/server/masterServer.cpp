...
StatusCode MasterServer::startMasterServer() {
    ...
    if (tetrisServer->startTetrisServer() != StatusCode::SUCCESS) {
        // I think both display the error message actually so it can be done this way
        printMessage("Failed to start TetrisServer", MessageType::CRITICAL);
        return StatusCode::ERROR;
    }

    // Start DBServer
    dbServer = std::make_shared<TetrisDBServer>("0.0.0.0", dbPort, "db-files/main.db");
...
StatusCode MasterServer::closeMasterServer() const {
...
StatusCode MasterServer::restartMasterServer() {
...
bool MasterServer::isTetrisServerRunning() const {
    return tetrisServer && (tetrisServer->isGameServerRunning() && tetrisServer->isLobbyServerRunning());
}

// DB stuff, completely forgot
bool MasterServer::isDBServerRunning() const {
    return dbServer && dbServer->isDBServerRunning();
}

void MasterServer::printMessage(const std::string &message, const MessageType msgtype) const {
...
int main(int argc, char *argv[]) {
    ...
    MasterServer masterServer(MASTER_SERVER_IP, LOBBY_SERVER_PORT, DB_SERVER_PORT, DEBUG);
