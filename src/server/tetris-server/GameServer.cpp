    ...
    debug(debug)
{
    ...
    // this will be used to create a new game server instance, using the lobbyServer data to initialize the game server
}
...
StatusCode GameServer::startGameServer() {
    // This method is used to start the game server. It should be called in a new thread.
    // It will start the game server and open the socket for the players to connect.
    // this will also start the game server loop, which will run until the game server is closed.
...
StatusCode GameServer::closeGameServer() {
    // This method is used to close the game server. It should be called in a new thread.
    // It will close the game server and close the socket for the players to connect.
    // this will also stop the game server loop, which will run until the game server is closed.
...
void GameServer::listen() {
    ...
    // It will run in a separate thread and will start the games when the lobbies are ready.
    ...
    while (true) {
        ...
        // if the game server is still running, we can get the ready lobbies and the empty lobbies
        std::vector<std::shared_ptr<Lobby> > readyLobbies = lobbyServer->getReadyLobbies();
        std::vector<std::shared_ptr<Lobby> > deadLobbies = lobbyServer->getDeadLobbies();

        // we kill the dead lobbies
        for (const auto &lobby: deadLobbies) {
            if (lobbyServer->closeLobby(lobby->getLobbyID()) != StatusCode::SUCCESS) {
                printMessage("Error closing lobby: " + lobby->getState().lobbyID, MessageType::ERROR);
            }
        }
        ...
        for (const auto &lobby: readyLobbies) {
            ...
            if (countGames() < MAX_GAMES) {
                ...
                printMessage("Cannot start a new game, maximum number of games reached", MessageType::WARNING);
                break;
            }
        }
    }
...
void GameServer::startGame(const std::shared_ptr<Lobby> &lobby) {
    ...
    if (lobbyServer->closeLobby(lobbyState.lobbyID) != StatusCode::SUCCESS) {
        printMessage("Error closing lobby: " + lobbyState.lobbyID, MessageType::ERROR);
        return;
    }
    ...
    if (gameStarted != StatusCode::SUCCESS) {
        printMessage("Error starting game: " + getStatusCodeString(gameStarted), MessageType::ERROR);
        return;
    }
...
void GameServer::printMessage(const std::string &message, MessageType msgType) const {
    ...
    std::cout << gameIdentifier << "[" << msgtype_str << "] " << message << std::endl;
