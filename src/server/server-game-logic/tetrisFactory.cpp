...
void TetrisFactory::fillPool() {
    ...
    if (pool.empty()) {
        ...
        pool.clear();
        for (auto p : newPool) { pool.emplace_back(Position2D{0, 0}, p); }
...
bool TetrisFactory::isPoolEmpty() const  { return pool.empty(); }
int TetrisFactory::getPoolSize() const   { return static_cast<int>(pool.size()); }
