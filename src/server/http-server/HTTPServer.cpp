...
TetrisHTTPServer::TetrisHTTPServer(std::string address, const unsigned short port)
...
TetrisHTTPServer::~TetrisHTTPServer() {
    TetrisHTTPServer::stop();
}

void TetrisHTTPServer::run() {
    running_.store(true);
    std::cout << "[HTTP] [INFO] TetrisHTTPServer listening on "
              << address_ << ":" << port_ << " (TCP/HTTP)" << std::endl;
...
void TetrisHTTPServer::stop() {
    if (running_.exchange(false)) {
        std::cout << "[HTTP] [INFO] Stopping server on " << address_ << ":" << port_ << std::endl;
...
void TetrisHTTPServer::doAccept() {
    if (!running_.load()) return; // Bail out
...
void TetrisHTTPServer::doSession(tcp::socket socket) {
    beast::error_code ec;
    ...
    for (;;) {
        ...
        if (ec) {
            std::cerr << "[HTTP] [ERROR] Read error: " << ec.message() << std::endl;
            break;
        }
        ...
        if (ec) {
            std::cerr << "[HTTP] [ERROR] Write error: " << ec.message() << std::endl;
            break;
        }
...
void TetrisHTTPServer::handleRequest(
    http::request<http::string_body>    req,
...
std::string TetrisHTTPServer::forwardRequest(
    ...
    const std::string& body)
{
    try {
        ...
        std::cerr << "[HTTP] [ERROR] Error forwarding request: " << e.what() << std::endl;
        return "";
    }
}